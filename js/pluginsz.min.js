...
(function ($, window, document, undefined) {
    ...
    Plugin.prototype = {
        init: function () {
            ...
            if (isObject(this.options.pofloorFloorName) && this.options.pofloorFloorName.length < this.nodeChildren.length) {
                return this._emitConsoleMessage("error", "floors total (" + this.nodeChildren.length + ") & floor name array length (" + this.options.pofloorFloorName.length + ") don't match");
            }
        ...
        setup: function () {
            ...
            if (isObject(this.options.pofloorFloorName)) {
        ...
        _bindEvents: function () {
            ...
            this.node.on('scrollToStage', function (event, floor) {
                if (typeof floor == 'string') {
                    var floorId = $.inArray(floor, self.options.pofloorFloorName);
            ...
            $(window).on('resize.pofloor', function (event) {
                self.scrollToFloor(self.floorActive, false);
            });
            if (isObject(this.options.pofloorFloorName)) {
                $(window).on('hashchange.pofloor', function (event) {
                    self._hashchangeHandler(event);
                });
            }
            if (window.DeviceOrientationEvent) {
                $(window).on('orientationchange.pofloor', function (event) {
                    self.scrollToFloor(self.floorActive);
                });
            }
            if (this.options.keyNavigation) {
                $(document).on('keydown.pofloor', function (event) {
                    self._keypressHandler(event);
                });
            }
            if (this.options.wheelNavigation) {
                this.node.on('mousewheel.pofloor', function (e) {
                    setTimeout(function () {
                        if (!self.scrollInChildren) self._handleMouseWheelEvent(e);
                    }, 10);
                });
                this.nodeChildren.on('scroll.pofloor', function (e) {
            ...
            if (this.options.swipeNavigation) {
                var touchEvent = 'touchstart.pofloor touchend.pofloor touchcancel.pofloor';
                if (this.options.swipeNavigation !== 'mobile-only') touchEvent += ' mousedown.pofloor mouseup.pofloor';
        ...
        destroy: function () {
            this.nodeChildren.off('scroll.pofloor');
            this.node.off('mousewheel.pofloor scrollToDirection scrollToStage next prev refresh remove touchstart.pofloor touchend.pofloor mousedown.pofloor mouseup.pofloor touchcancel.pofloor');
            $(window).off('resize.pofloor hashchange.pofloor orientationchange.pofloor');
            $(document).off('keydown.pofloor');
            this.node.css({
                'position': '',
                'overflow': '',
                'top': '',
        ...
        _getFloorFromHash: function () {
            if (this._getHash()) {
                if (this.options.pofloorFloorName && existInArray(this.options.pofloorFloorName, this._getHash())) {
                    return this.options.pofloorFloorName.indexOf(this._getHash());
                }
            }
            return false;
        },
        ...
        _updateHash: function (floorIndex) {
            if (isObject(this.options.pofloorFloorName) && this._getHash() !== this.options.pofloorFloorName[floorIndex]) {
                window.location.replace(('' + window.location).split('#')[0] + '#' + this.options.pofloorFloorName[floorIndex]);
            }
        },
        ...
        _emitConsoleMessage: function (type, warn) {
            if (type == "error") console.error("pofloor.js: " + warn);
            if (type == "warn") console.warn("pofloor.js: " + warn);
        },
        ...
        scrollToFloor: function (floor) {
            if (isString(floor) && existInArray(this.options.pofloorFloorName, floor)) floor = this.options.pofloorFloorName.indexOf(floor);
            var self = this;
    ...
    Plugin.prototype = {
        ...
        calculate: function () {
            var t = this,
                ...
                dateReached = false;
            t._changed = false;
            t.setTargetDate(new Date(s.year, s.month - 1, s.day, s.hour, s.minute, s.second));
        ...
        format: function () {
            ...
            t.minutesLabel = (i == 1 && siL !== null && sL == true) ? siL : s.minutesLabel, t.secondsLabel = (se == 1 && sseL !== null && sL == true) ? sseL : s.secondsLabel, t.decisecondsLabel = (ds == 1 && sdsL !== null && sL == true) ? sdsL : s.decisecondsLabel, t.millisecondsLabel = (m == 1 && smL !== null && sL == true) ? smL : s.millisecondsLabel;
        },
        output: function () {
            var t = this,
                s = t.settings;
...
pogradient = {
    ...
    create: function (startColor, endColor, steps) {
        ...
        if ((s.join("") + e.join("")).replace(/[0-f]/g, "") != "") {
            return
        };
    ...
    set: function (elements, startColor, endColor) {
        ...
        for (var i = 0; i < x.length; i++) {
            ...
            if (elements) {
                c = ["gradient", startColor, endColor]
            };
...
particleImage.src = 'media/snow.png';
...
function loop() {
    ...
    f.pobubble = function (l, t) {
        ...
        h.data("pobubble", a);
        a.init = function () {
            a.options = f.extend({}, f.pobubble.defaultOptions, t);
            h.css({
    ...
    f.fn.pobubble = function (l) {
        return this.each(function () {
            new f.pobubble(this, l)
        })
    }
})(jQuery);